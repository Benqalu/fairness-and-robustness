...
class PreProcFlip(object):
	def __init__(self, data, attr, method='FGSM', k=10, seed=None, max_iter=1000, dR=0.1, dF=0.1):
		...
		self._train = {
			...
			'y_orig':torch.tensor(self._train_np['y'].reshape(-1,1), dtype=torch.float),
	...
	def _Hessian(self, model, X_train, y_train):
		...
		grad_L_w_1 = gradient(loss, (weights, bias), retain_graph=True, create_graph=True)
		Hessian = []
		for i in range(0, grad_L_w_1[0].shape[1]):
			grad_L_w_2 = gradient(grad_L_w_1[0][0][i], (weights, bias), retain_graph=True)
	...
	def fit_transform(self, test_output=True):
		...
		for it in range(0, self._max_iter):
			...
			orig_grad_utility = gradient(orig_loss_utility, (weights, bias), retain_graph=True)
			orig_grad_utility = np.array(orig_grad_utility[0][0].tolist()+orig_grad_utility[1].tolist())
			# END: Common calculations

			# BEGIN: Pre-processing of fairness
			infl = -np.ones(self._train['s'].shape[0])
			if len(res["test"]["disp"])>0 and res["test"]["disp"][-1] > self._dF and len(chosen_F) < self._train['X'].shape[0]:
				y_pred = model(X_train)
				...
				for i in range(0, self._train['y'].shape[0]):
					...
					grad_loss_orig = gradient(loss_orig[i], (weights, bias), retain_graph=True)
					grad_loss_orig = np.array(grad_loss_orig[0][0].tolist()+grad_loss_orig[1].tolist())
					
					grad_loss_flip = gradient(loss_flip[i], (weights, bias), retain_graph=True)
					grad_loss_flip = np.array(grad_loss_flip[0][0].tolist()+grad_loss_flip[1].tolist())
				...
				orig_grad_fairness = gradient(orig_loss_fairness, (weights, bias), retain_graph=True)
				orig_grad_fairness = np.array(orig_grad_fairness[0][0].tolist()+orig_grad_fairness[1].tolist())

				for i in range(0, self._train['y'].shape[0]):
					infl[i] = np.dot(orig_grad_fairness,I_theta[i])*np.exp(-abs(np.dot(orig_grad_utility,I_theta[i])))
			...
			infl = -np.ones(self._train['s'].shape[0])
			if len(res["test"]["attk"])>0 and res["test"]["attk"][-1] < self._dR and len(chosen_R) < self._train['X'].shape[0]:
				
				X_adv = self.AdvExp(X=self._train['X'], y=self._train['y'], method=self._method)
				y_adv_pred = model(X_adv)
				loss_robustness = self._BCELoss(y_adv_pred, self._train['y'], reduction=False)

				I_theta = []

				for i in range(0, self._train['X'].shape[0]):
					grad_loss = gradient(loss_robustness[i], (weights, bias), retain_graph=True)
					grad_loss = np.array(grad_loss[0][0].tolist()+grad_loss[1].tolist())
				...
				orig_grad_robustness = gradient(orig_loss_robustness, (weights, bias), retain_graph=True)
				orig_grad_robustness = np.array(orig_grad_robustness[0][0].tolist()+orig_grad_robustness[1].tolist())

				for i in range(0, self._train['y'].shape[0]):
					infl[i] = np.dot(orig_grad_robustness,I_theta[i])*np.exp(-abs(np.dot(orig_grad_utility,I_theta[i])))

...
class PreProcFlip(object):
	...
	def fit_transform(self, test_output=True):
		...
		for it in range(0, self._max_iter):
			...
			if ((len(res["test"]["disp"])==0) or (len(res["test"]["disp"])>0 and res["test"]["disp"][-1] > self._dF)):
			...
			if len(res["test"]["attk"])==0 or res["test"]["attk"][-1] < self._dR:
				# BEGIN: Pre-processing of robustness
				if len(chosen_R) == self._train['X'].shape[0]: # Rolling
					chosen_R = chosen_R[self._k:]
					self._train['X_adv'] = self._train['X_adv'][self._k:,:].detach()

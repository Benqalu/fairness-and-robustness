...
class PreProcFlip(object):
	def __init__(self, data, attr, method='FGSM', k=10, lr=0.01, seed=None, max_iter=1000, dR=0.1, dF=0.1):
		self._data = data
		self._attr = attr
		self._seed = seed
	...
	def fit_transform(self, test_output=True):
		...
		for it in range(0, self._max_iter):
			...
			if self._method == 'FGSM':
				...
				influence_to_robustness = - y_grad
			...
			if acc_train_atk < 1.0 - self._dR:
				rlist = np.argsort(abs(influence_to_robustness))[::-1]
				rest = self._k
				for i in rlist:
					if influence_to_robustness[i] * influence_to_fairness[i] > 0 and 

...
class Experiments(object):
	...
	def exec(self):
		...
		if "positive" in self._prefix:
			model = FaroLR(
				fairness=self._alpha,
				robustness=self._beta,
				lr=1e-3,
				n_epoch=n_epoch,
				bias=self._bias,
				report=[
		...
		if self._recording:
			f = open(
				f'./result/{self._prefix}/report_{self._data}_{self._attr}_f{"%03d"%int(self._alpha*100)}_r{"%03d"%int(self._beta*100)}_ep{self._n_epoch}_{self._timestamp}.txt',
				"w",
			)
		...
		if self._recording:
			...
			plt.savefig(
				f'./result/{self._prefix}/angle_{self._data}_{self._attr}_f{"%03d"%int(self._alpha*100)}_r{"%03d"%int(self._beta*100)}_ep{self._n_epoch}_{self._timestamp}.pdf'
			)
			# plt.show()

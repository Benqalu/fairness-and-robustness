        ...
        for data in datas:
            for attr in attrs:
                for dF in dFs:
                    for dR in dRs:
                        if (
                            dF < ranges[(data, attr)]["F"][0]
                            or dF > ranges[(data, attr)]["F"][1]
                        ):
                            continue
                        if (
                            dR < ranges[(data, attr)]["R"][0]
                            or dR > ranges[(data, attr)]["R"][1]
                        ):
                            continue
                    ...
                    for dF in dFs:
                        for dR in dRs:
                            ...
                            if combo in executed and executed[combo] > 0:
                                executed[combo] -= 1
                                print(
                                    "Combo %s is already calculated, skip." % str(combo)
                                )
                                continue
                            ...
                            os.system(
                                "python PreProcessInflu.py %s %s %s %.2f %.2f"
                                % (data, attr, method, dF, dR)
                            )
                            count += 1
                            time.sleep(1)

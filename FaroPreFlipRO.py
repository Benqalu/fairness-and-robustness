...
class PreProcFlip(object):
	...
	def _BCELoss(self,y_pred,y):
		return -torch.mean(y*torch.log(0.99 * y_pred)+(1.0-y)*torch.log(1.0-0.99 * y_pred))
	...
	def fit_transform(self, test_output=True):
		...
		optim=torch.optim.Adam(model.parameters(), lr=0.1, weight_decay=1E-4)
		...
		for it in range(0, self._max_iter):
			...
			acc_train_org = Metric(true=y_np, pred=y_pred.detach().numpy().reshape(-1)).accuracy()
			...
			print('Iter: %d, Train: (%.4f, %.4f)'%(it+1, acc_train_org, acc_train_atk), end=', ')
			...
			if test_output:
				...
				acc_test_org = Metric(true=y_test_np, pred=y_test_pred.detach().numpy().reshape(-1)).accuracy()
				...
				acc_test_atk = Metric(true=y_test_np, pred=y_test_pred_atk.detach().numpy().reshape(-1)).accuracy()

				print('Test: (%.4f, %.4f)'%(acc_test_org, acc_test_atk))
				res['test']['orig'].append(acc_test_org)
				res['test']['attk'].append(acc_test_atk)
			# END: Testing

			# BEGIN: Flipping
			influence=np.column_stack((influence_to_robustness, influence_to_utility, list(range(0,influence_to_utility.shape[0]))))
...
def draw(res):
	...
	data=np.array(res['train']['orig']).reshape(-1,1)
	df_train=DataFrame(data, index=res['iter'], columns=['Original Acc.'])
	main_ax=df_train.plot(ax=ax[0])
	main_ax.set_xlabel('Training epochs')
	main_ax.set_ylabel('Original Accuracy')
	main_ax.set_title(res['setting']+'_train')

	data=np.array(res['train']['attk']).reshape(-1,1)
	df_train=DataFrame(data, index=res['iter'], columns=['Attacked Acc.'])
	m2nd_ax=df_train.plot(secondary_y=True, ax=main_ax)
	m2nd_ax.set_ylabel('Attacked Accuracy')

	data=np.array(res['test']['orig']).reshape(-1,1)
	df_train=DataFrame(data, index=res['iter'], columns=['Original Acc.'])
	...
	data=np.array(res['test']['attk']).reshape(-1,1)
	df_train=DataFrame(data, index=res['iter'], columns=['Attacked Disp.'])
	m2nd_ax=df_train.plot(secondary_y=True, ax=main_ax)
	m2nd_ax.set_ylabel('Attacked Accuracy')
...
if __name__=='__main__':
	model = PreProcFlip('adult','race',max_iter=3,k=10,credit_lim=1)

...
class PreProcFlip(object):
	...
	def fit_transform(self, test_output=True):
		...
		for it in range(0, self._max_iter):
			...
			y_pred = model(self._train['X'])
			loss = self._BCELoss(y_pred, self._train['y'], reduction=False).detach().numpy().reshape(-1)
			loss_ = self._BCELoss(y_pred, 1-self._train['y'], reduction=False).detach().numpy().reshape(-1)
			influence_to_utility = (loss_ - loss).tolist()
			...
			loss_ = self._BCELoss(y_adv_pred, 1-self._train['y'], reduction=False).detach().numpy().reshape(-1)
			influence_to_robustness = (loss_ - loss).tolist()
			
			acc_train_atk = Metric(true=self._train_np['y'], pred=y_adv_pred.detach().numpy().reshape(-1)).accuracy()

			ranking_utility = np.zeros(self._train['y'].shape[0])
			ranking_utility[np.argsort(influence_to_robustness)] = np.arange(0, self._train['y'].shape[0]) + 1
			...
			if test_output:
				...
				Mode = None
				if (1.0 - acc_test_atk) > self._dR and acc_test_atk < acc_test_org - 0.005:
					influence_score = influence_to_robustness
					Mode = 'Robustness'
				elif disp_test_org > self._dF:

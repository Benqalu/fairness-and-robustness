	...
	def fit_transform(self, test_output=True):
		...
		for it in range(0, self._max_iter):
			...
			if disp_train_org > self._delta:
				...
				influence_to_fairness = np.sign(disp*(s_np-0.5)*(y_np_prime-0.5))
			else:
			...
			print('Iter: %d, Train: (%.4f, %.4f, %.4f)'%(it+1, acc_train_org, acc_train_atk, disp_train_org), end=', ')
			...
			if test_output:
				...
				metric = Metric(true=y_test_np, pred=y_test_pred.detach().numpy().reshape(-1))
				...
				print('Test: (%.4f, %.4f, %.4f)'%(acc_test_org, acc_test_atk, disp_test_org))
			...
			influence=np.column_stack((influence_to_fairness, influence_to_robustness, influence_to_utility, list(range(0,influence_to_utility.shape[0]))))
...
def draw(res):
	...
	data=np.hstack([np.array(res['train']['orig']).reshape(-1,1), np.array(res['train']['attk']).reshape(-1,1)])
	df_train=DataFrame(data, index=res['iter'], columns=['Accuracy_Orig.', 'Accuracy_Attk.'])
	...
	data=np.hstack([np.array(res['test']['orig']).reshape(-1,1), np.array(res['test']['attk']).reshape(-1,1)])
	df_test=DataFrame(data, index=res['iter'], columns=['Accuracy_Orig.', 'Accuracy_Attk.'])
...
if __name__=='__main__':
	model = PreProcFlip('adult', 'race', max_iter=100, k=10, credit_lim=1)

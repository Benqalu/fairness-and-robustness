...
class FaroInProc(TorchNeuralNetworks):
	def __init__(self, lr=0.01, n_epoch=1000, method='FGSM', eps=0.1, hiddens=[], seed=None):
		super(FaroInProc, self).__init__(lr=lr, n_epoch=n_epoch, hiddens=hiddens, seed=seed)
		self._seed = seed
	...
	def fit(self, X, y, s=None, wR=0.0, wF=0.0, test=None, rep=False):
		...
		if s is not None:
			self._s = torch.tensor(s.reshape(-1, 1), dtype=torch.float)
			self._c = (1-self._s-self._s)/((1-self._s)*torch.sum(1-self._s) + self._s*torch.sum(self._s))

		self._model = TorchNNCore(
			inps=self._X.shape[1], hiddens=self._hiddens, seed=self._seed
		)
		optim = torch.optim.Adam(
			self._model.parameters(), lr=self._lr, #weight_decay=wR
		)
		...
		for epoch in tqdm.tqdm(range(self._n_epoch)):
			...
			if rep and (epoch % 5 == 1 or epoch == self._n_epoch - 1):
				...
				report['params'].append([item.tolist() for item in self._model.parameters()])
				...
				report['train_metric_attack'].append(self.metrics_attack(X,y,s,method=self._method,use_y=True))
				report['test_metric'].append(self.metrics(test['X'],test['y'],test['s']) if test is not None else None)
				report['test_metric_attack'].append(self.metrics_attack(test['X'],test['y'],test['s'],method=self._method,use_y=False) if test is not None else None)

				optim.zero_grad()
				loss_u = self._loss_func(self._model(self._X), self._y)
				loss_u.backward()
				report['grad_u'].append([item.grad.tolist() for item in self._model.parameters()])
				...
				if self._method=='FGSM':
					...
					raise RuntimeError('Method must be FGSM or PGD.')
				loss_r.backward()
				report['grad_r'].append([item.grad.tolist() for item in self._model.parameters()])

				optim.zero_grad()
				loss_f = self.loss_fairness(self._model(self._X), self._c)
				loss_f.backward()
				report['grad_f'].append([item.grad.tolist() for item in self._model.parameters()])
...
def experiments(data, attr, method='FGSM', wR=0.0, wF=0.0, seed=None, suffix=''):
	...
	report = model.fit(train['X'], train['y'], s=train['s'], wR=wR, wF=wF, test=test, rep=True)
	report['data']=data
	report['attr']=attr

...
class AdversarialDebiasing(Transformer):
	...
	def fit(self, dataset):
		...
		with tf.variable_scope(self.scope_name):
			...
			if self.debias:
				...
				(
					pred_protected_attributes_labels,
					pred_protected_attributes_logits,
			...
			learning_rate = tf.train.exponential_decay(
				starter_learning_rate, global_step, 1000, 1.00, staircase=True
			)
	...
	def predict(self, dataset):
		...
		batch_feed_dict = {
			self.features_ph: dataset.features,
			self.true_labels_ph: dataset.labels.reshape(-1, 1),
			self.protected_attributes_ph: dataset.protected_attributes[
				:,
	...
	def attack(self, dataset, method="FGSM", use_label=False, epsilon=0.1):
		features = np.array(dataset.features)
		if use_label:
			true_labels = dataset.labels
		else:
			true_labels = self.predict(dataset).labels
		...
		if method == "FGSM":
			...
			noise = (
				np.sign(self.sess.run(self.noise_score, feed_dict=feed_dict)[0])
				* epsilon
			)
			dataset_new = dataset.copy(deepcopy=True)
			...
			for i in range(0, 10):
				...
				noise = np.sign(
					self.sess.run(self.noise_score, feed_dict=feed_dict)[0]
				) * (epsilon / 10)
				features += noise
			dataset_new = dataset.copy(deepcopy=True)

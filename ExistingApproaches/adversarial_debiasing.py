...
class AdversarialDebiasing(Transformer):
    ...
    def fit(self, dataset):
        ...
        self.noise_score = tf.gradients(pred_labels_loss, self.features_ph)
    ...
    def predict(self, dataset):
        ...
        batch_feed_dict = {self.features_ph: dataset.features,
                           self.true_labels_ph: dataset.labels.reshape(-1,1),
                           self.protected_attributes_ph: dataset.protected_attributes[:,dataset.protected_attribute_names.index(self.protected_attribute_name)].reshape(-1,1),
                           self.keep_prob: 1.0}
    ...
    def attack(self, dataset, method='FGSM', epsilon=0.1):
        features = np.array(dataset.features)
        true_labels = dataset.labels
        protected_attributes = np.reshape(dataset.protected_attributes[:,
                dataset.protected_attribute_names.index(self.protected_attribute_name)], [-1,1])
        if method=='FGSM':
            feed_dict = {self.features_ph: features,
                       self.true_labels_ph: true_labels.reshape(-1,1),
                       self.protected_attributes_ph: protected_attributes,
                       self.keep_prob: 1.0}
            noise = np.sign(self.sess.run(self.noise_score, feed_dict=feed_dict)[0]) * epsilon
            dataset_new = dataset.copy(deepcopy = True)
            dataset_new.features += noise
            return dataset_new
        elif method=='PGD':
            for i in range(0,10):
                feed_dict = {self.features_ph: features,
                       ...
                       self.protected_attributes_ph: protected_attributes,
                       self.keep_prob: 1.0}
                noise = np.sign(self.sess.run(self.noise_score, feed_dict=feed_dict)[0]) * (epsilon/10)
                features += noise
            dataset_new = dataset.copy(deepcopy = True)

...
exp_params.dataset_name = '2Ddata';
exp_params.test_fraction = 0.2; %fraction of randomly chosen test data (the rest will be used for training)
...
exp_params.dataset_folder = 'exp2D/data'; %This is the folder in which the dataset is stored


classifier_params.classifier = classifier; % 'SVM_lin' or 'MLP'
classifier_params.xval=0; 
classifier_params.mlp_steepness = 0.0005;
classifier_params.neurons = neurons;
...
constraint_params.constraint = 'box_fixed';   % alternatively: 'box', 'box_fixed'
constraint_params.constraint_step = 1.5; % if 'box' is set as the constraint, this sets a 3x3 box centered on the initial attack sample
constraint_params.max_boundaries = 1;
constraint_params.lb=-4; %these values define lower and upper bounds for 'box_fixed'
constraint_params.ub=+4;

...
constraint_params.max_boundaries = 5;

plot_params.title=['Example' ...
        ', \lambda=' num2str(mimicry_params.lambda)]; %Set up plot title
plot_params.legend_lk = ['SVM RBF' ' - LK' ' (\gamma=' num2str(classifier_params.svm_gamma) ')']; %Set up legend for lk
plot_params.legend_pk = ['SVM RBF' ' - PK' ' (\gamma=' num2str(classifier_params.svm_gamma) ')']; %Set up legend for pk
plot_params.color = 'b';

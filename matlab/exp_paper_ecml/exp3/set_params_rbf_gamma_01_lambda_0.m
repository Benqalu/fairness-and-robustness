...
exp_params.test_fraction = 0.5;
exp_params.nfolds = 3;
exp_params.nsplits = 5;
...
classifier_params.classifier = 'SVM_rbf'; % 'SVM_rbf' or 'SVM_lin'
classifier_params.xval=0;
classifier_params.mlp_steepness = 0.0005;
classifier_params.neurons = 5;
classifier_params.svm_C = 1;
classifier_params.svm_gamma = 0.1;

evil_classifier_params.training_size = 100; 
evil_classifier_params.num_rep = 2; %Number of classifier's copies to learn by randomly sampling a training set
...
plot_params.title=['SVM (RBF)' ...
        ', \lambda=' num2str(mimicry_params.lambda)]; %Set up plot title
plot_params.legend_lk = ['LK' ' (C=' num2str(classifier_params.svm_C) ')']; %Set up legend for lk
plot_params.legend_pk = ['PK' ' (C=' num2str(classifier_params.svm_C) ')']; %Set up legend for pk
plot_params.color = 'r';
